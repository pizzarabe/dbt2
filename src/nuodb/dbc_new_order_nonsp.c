...
int  new_order(struct db_context_t *dbc, struct new_order_t *data, char ** vals, int nvals)
{
    ...
    sprintf(query, NUODB_NEW_ORDER_2, w_id, d_id);
...
/*
* NEW_ORDER3 is superfluous with a sequence
*/
    ...
    sprintf(query, NEW_ORDER_6, vals[D_NEXT_O_ID] , d_id, w_id, c_id, current_timestamp, o_ol_cnt, o_all_local);
